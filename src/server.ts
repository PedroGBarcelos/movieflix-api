...
type MovieBody = Record<string, unknown>
...
const isRecord = (value: unknown): value is MovieBody =>
    typeof value === 'object' && value !== null && !Array.isArray(value)

const parseMovieBody = (
    body: unknown,
    partial: boolean
): { data?: Prisma.MovieUncheckedCreateInput; error?: string } => {
    if (!isRecord(body)) {
    ...
    const hasUnknownField = keys.some(
        (key) => !movieFields.includes(key as (typeof movieFields)[number])
    )
    ...
    for (const field of ['genre_id', 'language_id'] as const) {
    ...
    if ('oscar_count' in body) {
        if (
            typeof body.oscar_count !== 'number' ||
...
app.post('/movies', async (req, res) => {
    ...
    if (error || !data) return res.status(400).send({ message: error })
